# Deploying VMAX Machine Ltd to Vercel
...
| `/api/*` (machines, services, careers, contact, chat, inbound mail) | One Vercel serverless function (`api/[...path].js`) |
...
**No artwork ships with this site.** Every image slot names the file it wants and
draws a labelled, hazard-striped plate carrying that name until the file exists, so
the site is presentable while you are still collecting photographs and nothing
reflows when they land. Adding them needs no code change: drop the files into
`public/assets/img/` (logos into `public/assets/brand/`) and the next build picks
them up. Any of `.webp`, `.avif`, `.jpg`, `.jpeg`, `.png` works, and `.webp` is
chosen first if more than one is present.

### Machine photographs

Fifty machines across seventeen classes. You can cover the whole catalogue with
**seventeen files**, one per class, and add per-machine photographs later.

| File | Covers |
| ---- | ------ |
| `vmax-wheel-loaders` | Every wheel loader |
| `vmax-excavators` | Every excavator |
| `vmax-dozers` | Every dozer |
| `vmax-articulated-haulers` | Every hauler |
| `vmax-motor-graders` | Every grader |
| `vmax-backhoe-loaders` | Every backhoe |
| `vmax-compaction` | Every roller |
| `vmax-telehandlers` | Every telehandler |
| `vmax-tractors` | Every tractor |
| `vmax-drill-rigs` | Every drill rig |
| `vmax-aerial-lifts` | Every boom and scissor lift |
| `vmax-forklifts` | Every forklift |
| `vmax-skid-steers` | Every skid steer |
| `vmax-cranes` | Every crane |
| `vmax-site-dumpers` | Every dumper |
| `vmax-crushing-screening` | Every crusher and screen |
| `vmax-power-lighting` | Every generator and lighting tower |

A photograph for one specific machine overrides its class picture: name it
`vmax-<brand>-<model>`, for example `vmax-caterpillar-320-gc.jpg` or
`vmax-john-deere-6120m.jpg`. Every machine in the catalogue is a real model, so
the name on the card is the name to search for.
`public/assets/img/README.txt` lists all fifty file names beside the machine
each one belongs to. Cut-outs on a white background sit best: the card places
them with `object-fit: contain`.

### Hero, section and page artwork

Landscape photographs of machines working, the yard and the workshop.

| File | Where it appears | Suggested size |
| ---- | ---------------- | -------------- |
| `vmaxhero1` | First hero slide | 2400 x 1400 |
| `vmaxhero2` | Second hero slide | 2400 x 1400 |
| `vmaxhero3` | Third hero slide | 2400 x 1400 |
| `vmaxfleet` | Equipment range section | 1920 x 1200 |
| `vmaxyard` | Why VMAX section | 1920 x 1200 |
| `vmaxworkshop` | Services section and the `/services` header | 1920 x 1200 |
| `vmaxparts` | Parts and attachments artwork | 1920 x 1200 |
| `vmaxcontact` | Contact section and the `/contact` header | 1920 x 1200 |
| `vmaxmachines` | The `/machines` header | 1920 x 1200 |
| `vmaxcareers` | The `/careers` header | 1920 x 1200 |

The hero slides carry the most weight: pick ones with calm space rather than busy
detail on the left, where the headline sits.

### Logo and icons

| File | Where | Notes |
| ---- | ----- | ----- |
| `public/assets/brand/vmaxlogo` | Site header and footer | For dark backgrounds: white or yellow type |
| `public/assets/brand/vmaxlogo-black` | The staff desk at `/admin` | For light backgrounds: black type |
| `public/favicon.svg` | Browser tab | Drawn already: the V mark |
| `public/favicon.png` | Browser tab | 512 x 512, drawn already |
| `public/favicon.ico` | Browser tab, legacy | 32 x 32, drawn already |
| `public/apple-touch-icon.png` | iOS home screen | 180 x 180, drawn already |

The icons are generated by `npm run gen:favicon` from `scripts/make-favicon.js`,
so they are the only artwork that ships. Replace them only if you want a
different mark.

Until a logo file exists the wordmark is set in type, which is a finished mark
rather than a gap, so there is no rush on it.

### How the mapping works

`src/data/images.json` holds the slot list and `src/site/images.js` resolves it at
build time. Machine and service photographs are named per item, in the `imageName`
field of `src/data/machines.json` and `src/data/services.json`, so you can rename
them to anything you like as long as the data matches.

The build prints both what it found and what it is still waiting for:

```
[build] awaiting 12 image(s) in public/assets: vmaxlogo, vmaxlogo-black, vmaxfleet, ...
[build] each one is drawn as a labelled plate until the file is dropped in.
```

`public/assets/img/README.txt` lists every name beside the machine it belongs to, so
the folder explains itself to whoever is doing the uploading.

Notes:

- Names are matched without regard to case, so `VMAX3.JPG` and `vmax3.jpg` both
  work. That matters: the deploy runs on Linux, where the two are different files.
- Keep files under about 400 KB each. Save a WebP copy beside the original (same
  name, `.webp`) and it is used in preference automatically.
- On Vercel the resolution happens at build time, so a newly uploaded image
  appears after the next deploy, not on the next request.
...
3. Leave the build settings alone. `vercel.json` already sets the build command,
   the `public` output directory, clean URLs and the `/machines/:id` rewrite, and
...
The function logs `[vmax] storage: Supabase` on the next request. Confirm what the
...
3. In Vercel, add:
   ...
   | `FORM_FROM` | `VMAX Website <onboarding@resend.dev>` to start |
...
`VMAX Website <website@vmaxmachineltd.com>`. Mail from a verified domain is far less
...
2. `/machines` lists the inventory; open one for the full specification.
